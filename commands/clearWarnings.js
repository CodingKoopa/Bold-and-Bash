...
const callback = function(args, message) {
  ...
  message.mentions.users.forEach((user) => {
    ...
    let warnings = app.warnings.filter(x => x.id == user.id && !x.cleared);
    const logMessage = `${authorInfo} has cleared warnings for ${userInfo} (${warnings.length} \
warnings).`;
    logger.info(logMessage);
    app.logChannel.send(logMessage);
    message.channel.send(`${message.author} clearing warnings for ${user} \
(${user.username})`);
